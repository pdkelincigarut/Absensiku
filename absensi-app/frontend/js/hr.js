...
function hrTabButton(id, label) {
  ...
  return `<button data-tab="${id}" class="hr-tab-btn whitespace-nowrap px-3 py-2 text-sm font-medium rounded-t-lg border-b-2 transition ${active ? 'border-klc-600 text-klc-600' : 'border-transparent text-slate-500 hover:text-slate-700'}">${label}</button>`;
}
...
async function renderHrTab(account) {
  ...
  document.querySelectorAll('.hr-tab-btn').forEach(btn => {
    ...
    btn.className = `hr-tab-btn whitespace-nowrap px-3 py-2 text-sm font-medium rounded-t-lg border-b-2 transition ${active ? 'border-klc-600 text-klc-600' : 'border-transparent text-slate-500 hover:text-slate-700'}`;
  });
